...
golang的`gin`库, 包含`app.env`环境变量文件的`Dockerfile`:
```
...
# 设置容器启动时执行的命令
ENTRYPOINT ["DB_SOURCE=postgresql://postgres:postgres@116.213.43.180:5432/simple_bank?sslmode=disable"]
CMD ["/bin/main"]
...
对应的`compose`, 该后端使用`postgres`数据库:
```yml
services:
